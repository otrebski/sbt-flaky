...
class FlakySpec extends WordSpec with Matchers {
  ...
  private val flakyReportDirSuccessful: File = new File(
    "./src/test/resources/flakyTestRuns/successful/target/flaky-report/"
  )
  private val successfulReport: FlakyTestReport = Flaky.createReport(
    "P1",
    TimeDetails(0, 100),
    List("1", "2"),
    flakyReportDirSuccessful
  )
  private val flakyReportDirWithFailures: File = new File(
    "./src/test/resources/flakyTestRuns/withFailures/target/flaky-report/"
  )
  private val failedReport: FlakyTestReport = Flaky.createReport(
    "P1",
    TimeDetails(0, 100),
    List("1", "2", "3"),
    flakyReportDirWithFailures
  )
  private val flakyReportDirAllFailures: File = new File(
    "./src/test/resources/flakyTestRuns/allFailures/target/flaky-test-reports/"
  )
  private val flakyReportAllFailures: FlakyTestReport = Flaky.createReport(
    "P1",
    TimeDetails(0, 100),
    List("1", "2", "3", "4", "5"),
  ...
  "Flaky" should {
    ...
    "create report based on 2 successful runs" in {
      ...
      successfulReport.flakyTests should contain(
        FlakyTest(
          Test("ExampleSpec", "A Stack should fail randomly often"),
          2,
          List.empty
        )
      )
      successfulReport.flakyTests should contain(
        FlakyTest(
          Test(
            "TestKitUsageSpec",
            "A ForwardingActor should Forwards in a 2 huge chains"
          ),
          2,
          List.empty
        )
    ...
    "create report based on 1 run and dir contains 2" in {
      val successfulReport = Flaky.createReport(
        "P1",
        TimeDetails(0, 100),
        List("1"),
        flakyReportDirSuccessful
    ...
    "create report based on run with failures" in {
      ...
      failedReport.flakyTests
        .find(_.test.test == "A Stack should fail sometimes")
        .map(_.failures()) shouldBe Some(1)
      failedReport.flakyTests
        .find(
          _.test.test == "A ForwardingActor should Forwards in a 2 huge chains"
        )
        .map(_.failures()) shouldBe Some(1)
    }
    ...
    "find failed tests in failed reports" in {
      val failedTestCase = TestCase(
        "run2",
        Test("t", "t2"),
        1f,
        Some(FailureDetails("msg", "ftype", "stacktrace"))
      )
      val runs: Seq[TestRun] = List(
        TestRun(
          "run1",
        ...
        TestRun(
          "run2",
          List(TestCase("run2", Test("t", "t1"), 1f, None), failedTestCase)
        )
      )
    ...
    "parse successful junit xml report" in {
      val expected = Set(
        ...
        TestCase(
          "run1",
          Test("ExampleSpec", "A Stack should fail sometimes"),
          0.002f,
          None
        ),
        TestCase(
        ...
        TestCase(
          "run1",
          Test("ExampleSpec", "A Stack should fail randomly"),
          0.001f,
          None
        ),
        TestCase(
          "run1",
          Test("ExampleSpec", "A Stack should fail randomly sometimes"),
          0.0f,
          None
        )
      )
      ...
      val testCases: Seq[TestCase] =
        Flaky.parseJunitXmlReport("run1", XML.loadFile(file))
    ...
    "group flaky cases in report with failure" in {
      ...
      val flakyCases = List(
        FlakyCase(
          ...
          Set("expected:<00:00:00.00[0]> but was:<00:00:00.00[3]>")
        ),
        FlakyCase(
          Test("tests.DateFormattingTest", "formatParallelTest"),
          List("1", "3", "4", "5"),
          ...
          Set(
            "expected:<00:00:00.00[2]> but was:<00:00:00.00[0]>",
            "expected:<00:00:00.00[1]> but was:<00:00:00.00[0]>",
            "expected:<00:00:00.00[2]> but was:<00:00:00.00[1]>"
          )
        )
      )
      println(s"\n\n${flakyCases.mkString("\n")}")
